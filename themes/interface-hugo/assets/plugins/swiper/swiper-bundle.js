...
(function (global, factory) {
  ...
  function EffectCoverflow(_ref) {
    ...
    const setTranslate = () => {
      ...
      const cTranslateY = params.depthY; // Each slide offset from center
      ...
      for (let i = 0, length = slides.length; i < length; i += 1) {
        ...
        let customTranslateY = -cTranslateY * Math.abs(offsetMultiplier);
        ...
        if (Math.abs(customTranslateY) < 0.001) customTranslateY = 0;
        if (Math.abs(rotateY) < 0.001) rotateY = 0;
        if (Math.abs(rotateX) < 0.001) rotateX = 0;
        if (Math.abs(scale) < 0.001) scale = 0;
        const slideTransform = `translate3d(${translateX}px,${customTranslateY}px,${translateZ}px)  rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(${scale})`;
        const $targetEl = effectTarget(params, $slideEl);
